# Polaris Bot 自循环体系（PAA 循环）架构设计文档 v4.1
...
| 4.1  | 2026-05-07 | BaseApplication 替换为 Protocol 鸭子类型；state.py 引入 activity_rate / activity_variability（心率/HRV 类比）；context_builder 新增状态感受翻译层 |
...
│   │       state.py             ← BotState 单例（activity_rate、activity_variability）
...
│   │       application_protocol.py ← ApplicationProtocol 鸭子类型定义（非基类）
│   │
│   ├───model/
...
### 5.2 ApplicationProtocol（鸭子类型，替代基类）

Application 不需要继承任何东西，只需要实现约定的方法。这更符合"每种能力都是独立的"这个哲学——她学会用 QQ 和学会写日记是两件完全不相干的事，不应该共享一个祖先类。

```python
# brain/platform/application_protocol.py
...
class ApplicationProtocol(Protocol):
    ...
    Application 的鸭子类型协议。
    不需要继承，只需要有这几个方法。
    ApplicationHost 用 isinstance(app, ApplicationProtocol) 做运行时校验。
    """
...
```python
# applications/qq/runtime.py
from pathlib import Path

class QQApplication:
    """不需要 import 任何 brain 模块，也不需要继承任何基类"""

    def manifest_path(self) -> Path:
        return Path(__file__).parent / "manifest.yaml"

    def on_start(self):
        # 初始化 OneBot 连接，注册事件监听
        ...
    ...
    def on_tick(self):
        # QQService 是事件驱动的，tick 里不需要做什么
        pass

    # 工具实现：由 ApplicationHost 在注册时绑定到 capability_registry
    async def send_qq_message(self, session_id: str, text: str) -> dict:
        ...
```
...
from brain.platform.application_protocol import ApplicationProtocol
...
class ApplicationHost:
    ...
    def register(self, app):
        ...
        if not isinstance(app, ApplicationProtocol):
            raise TypeError(f"{app.__class__.__name__} 未实现 ApplicationProtocol")
        ...
        # 注入平台 API（通过约定的 _bind 方法，如果 Application 选择使用的话）
        if hasattr(app, "_bind"):
...
### 6.2 state.py（心率与 HRV 类比）

精力系统删除后，BotState 引入两个真正有意义的生理节律指标：

| 指标 | 人类类比 | 含义 |
|------|---------|------|
| `activity_rate` | 心率 | 最近窗口内有多忙（事件频率） |
| `activity_variability` | HRV（心率变异性） | 事件到达的节律是否规律 |

两者组合描述四种状态：

```
activity_rate 高 + variability 高  →  活跃灵活（流动状态，最理想）
activity_rate 高 + variability 低  →  持续高压（专注但疲惫）
activity_rate 低 + variability 高  →  断续打扰（碎片化，难以专注）
activity_rate 低 + variability 低  →  真正平静（适合自维护）
```
...
class BotState:
    ...
    # 滑动窗口：最近 N 个心跳是否有事件（1=有，0=无）
    _recent_activity: deque = field(
        default_factory=lambda: deque(maxlen=20)
    )
    # 事件到达间隔序列（用于计算变异系数）
    _event_intervals: deque = field(
        default_factory=lambda: deque(maxlen=10)
    )
    _last_event_at: float = 0.0

    def record_tick(self, had_events: bool):
        """每心跳由 planner 调用，记录本轮是否有事件"""
    ...
    def activity_variability(self) -> float:
        """
        HRV：事件间隔的变异系数（std / mean）。
        越高说明事件到达越不规律（时而密集时而安静）。
        越低说明节律越稳定（持续忙碌或持续安静）。
        """
...
**这两个指标不暴露给 Application**，只在 brain 内部流动，经由 context_builder 翻译为自然语言感受后注入 LLM context。
...
async def run():
    ...
    # 记录本轮心跳是否有事件（驱动 activity_rate / variability 计算）
    bot_state.record_tick(had_events)
...
class ContextBuilder:
    ...
    def _bot_state_block(self) -> str:
        feeling = _describe_feeling(bot_state)
        # 普通状态不描述，减少 context 噪音
        if not feeling:
            return ""
        return f"## 你现在的状态\n{feeling}"

def _describe_feeling(state: "BotState") -> str:
    """
    指标 → 感受的翻译层。
    将机械的 activity_rate / variability 翻译成现象学描述，
    让 LLM 像感受自身状态一样读取，而不是解析数值。
    Application 永远看不到这里的指标，只有 LLM 通过感受描述感知到。
    """
    if state.is_stressed():
        return "今天一直很忙，消息一条接着一条，有点喘不过气。"

    if state.is_in_flow():
        return "今天的节奏挺好的，事情有来有往，不紧不慢。"

    rate = state.activity_rate
    var  = state.activity_variability

    if rate < 0.2 and var < 0.3:
        return "现在很安静，好一阵没什么动静了，适合做些自己的事。"

    if rate < 0.2 and var > 0.5:
        return "今天断断续续的，偶尔有人说话，但大多数时候都挺安静。"
...
活跃期（not is_idle）→ pop_highest()   专注高优先级
闲时期（is_idle）    → pop_lowest()    消化低优先级积压
```

`is_idle()` 的判断基于双指标：`activity_rate < 0.2 AND activity_variability < 0.3`，即真正的平静而非偶尔的安静。不使用时间老化机制，旧任务通过闲时拾取通路自然消化。

### 10.3 柔性任务（拟人化忽略）

`urgency: GENTLE` 的任务生成低优先级 Plan。LLM 在 expander 展开时读取 context 中的状态感受描述（"有点喘不过气" / "很安静，适合做自己的事"），自主决定是否忽略或推迟——不是靠固定概率，而是因为"感受到忙"才推迟。这比写死 `ignore_chance = 0.8` 更真实，也更符合数字生命的哲学。
...
| 变更项 | 类型 | 说明 |
|--------|------|------|
| `base_application.py` | 删除 | 替换为 Protocol 鸭子类型，Application 无需继承任何类 |
| `application_protocol.py` | 新增 | `@runtime_checkable` Protocol，ApplicationHost 用于运行时校验 |
| `BotState.idle_counter` | 删除 | 被 activity_rate + activity_variability 替代 |
| `BotState.activity_rate` | 新增 | 心率类比：最近窗口内事件频率 |
| `BotState.activity_variability` | 新增 | HRV 类比：事件到达间隔的变异系数 |
| `BotState.record_tick()` | 新增 | 由 planner 每心跳调用，更新两个指标 |
| `BotState.is_stressed()` | 新增 | 高频率 + 低变异 → 持续高压 |
| `BotState.is_in_flow()` | 新增 | 适中频率 + 高变异 → 流动状态 |
| `_describe_feeling()` | 新增 | 指标 → 自然语言感受的翻译层，注入 context |
| `context_builder._bot_state_block()` | 修改 | 从输出数值改为输出感受描述，Application 不可见 |
...
| activity_rate | BotState 的心率类比，最近窗口内事件频率，0~1 |
| activity_variability | BotState 的 HRV 类比，事件间隔变异系数，越高越不规律 |
| 状态感受翻译层 | `_describe_feeling()`，将机械指标翻译为自然语言，注入 context，Application 不可见 |
| ApplicationProtocol | 鸭子类型协议，替代 BaseApplication 基类，Application 无需继承 |

---

_v4.1 在 v4.0 的基础上完成了两处精化：一是用 Protocol 鸭子类型替代 BaseApplication 基类，让 Application 彻底自由；二是引入 activity_rate / activity_variability 双指标替代粗糙的 idle_counter，并通过感受翻译层将其注入 LLM context，使 Polaris 的状态以"感受"而非"数值"的方式影响她的行为——这是"数字生命"哲学在节律感知层面的完整表达。_
