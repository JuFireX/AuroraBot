...
> 用一个不太恰当的比喻：app 是记者（在外面跑、采集信息），platform 是编辑部（安排版面、排期），kernel 是排班经理（决定今天谁干活、先干啥），brain 是主编团队（真正动脑子写稿子）。虽然但是这个比喻还不如不比喻...
...
- 想了解脑区的架构：读 [脑区架构](./brain-architecture.html)
