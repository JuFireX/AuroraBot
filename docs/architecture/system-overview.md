...
- `platform` 层是应用的运行时宿主，它持有**应用层的事件队列**，负责应用发现、命令注册、事件缓存与生命周期管理，并维护与 App 层和 Kernel 层的双向通信。

- `kernel` 层是内核调度器，它持有**内核层的事件队列**，加载 `brain` 层的节点，管理节点的生命周期，并维护与 Platform 层和 Brain 层的双向通信。
