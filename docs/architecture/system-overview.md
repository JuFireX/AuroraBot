...
flowchart TB
    subgraph ROW1[" "]
        direction LR
        WORLD["外部世界"]
        APPS["应用层<br/>QQ / Alarm / Diary"]
        PLATFORM["平台运行时<br/>Host / API / Event Queue"]
    end

    subgraph ROW2[" "]
        direction LR
        KERNEL["内核流水线<br/>Plan / Expand / Execute"]
        COMMANDS["命令分发<br/>invoke_command()"]
        EFFECTS["外部效果<br/>消息 / 提醒 / 写日志"]
    end

    WORLD --> APPS --> PLATFORM
    PLATFORM -->|"事件进入内核"| KERNEL
    KERNEL --> COMMANDS --> EFFECTS
    EFFECTS -.->|"新的环境变化"| WORLD
```
