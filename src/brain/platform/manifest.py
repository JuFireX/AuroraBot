...
class ToolSpec:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "ToolSpec":
        return cls(
            ...
            side_effects=[str(item) for item in payload.get("side_effects", []) if str(item).strip()],
        )
...
class Manifest:
    ...
    def load(cls, path: Path) -> "Manifest":
        ...
        tools = [ToolSpec.from_dict(item) for item in payload.get("tools", []) if isinstance(item, dict)]
        return cls(
